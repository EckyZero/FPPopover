...
-(CGPathRef)newContentPathWithBorderWidth:(CGFloat)borderWidth arrowDirection:(FPPopoverArrowDirection)direction
{
    ...
    if(self.border == NO) {
        b = 10.0;
        if(self.tint == FPPopoverClearTint){
            b = 0.0;
        }
    }
...
-(CGGradientRef)newGradient
{
    ...
    else if(self.tint == FPPopoverClearTint){
        UIColor *color1 = [UIColor colorWithRed:0.f green:0.f blue:0.f alpha:0.f];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if(self.tint == FPPopoverClearTint){
        contentPath = [self newContentPathWithBorderWidth:0.0 arrowDirection:_arrowDirection];
    }
    ...
    CGContextAddPath(ctx, contentPath);
    if(self.tint == FPPopoverClearTint){
        CGContextSetRGBStrokeColor(ctx, 0.7, 0.7, 0.7, 0.0);
    } else {
    ...
    if(self.tint == FPPopoverClearTint){
        externalBorderPath = contentPath;
    }
    CGContextBeginPath(ctx);
